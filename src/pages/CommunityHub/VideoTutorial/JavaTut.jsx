...
import {JavaVideo} from "./dataSets"; 

import { Link } from "react-router-dom";

const JavaTut = () => {
  // Default Java logo image
  const javaImage = "https://upload.wikimedia.org/wikipedia/en/3/30/Java_programming_language_logo.svg";

  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 p-8">
      {JavaVideo.map((item, idx) => (
        <Link
          to={item.url}
          key={idx}
